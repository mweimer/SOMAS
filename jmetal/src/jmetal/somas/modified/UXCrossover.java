 ...
 * @version 1.0
 * Class representing a UX crossover operator
 */
...
/**
 * This class allows to apply a UX crossover operator using two parent
 * solutions.
 ...
 */
public class UXCrossover extends Crossover{
  ...
  /**
   * Constructor
   * Create a new instance of the UX crossover operator.
   */
  public UXCrossover() {
    try {
   ...
   /**
   * Constructor
   * Create a new intance of the UX crossover operator.
   */
   public UXCrossover(Properties properties) {
     this();
   } // HUXCrossover
        ...
        for (int var = 0; var < parent1.getDecisionVariables().length; var++) {
          ...
          for (int bit = 0; bit < p1.getNumberOfBits(); bit++) {
            if (p1.bits_.get(bit) != p2.bits_.get(bit)) {  
            	 if (PseudoRandom.randDouble() < bitSwapProbability) {
  ...
  */
  public Object execute(Object object) throws JMException {
    ...
    if ( ((parents[0].getType().getClass() != BINARY_SOLUTION) ||
      ...
      Configuration.logger_.severe("UXCrossover.execute: the solutions " +
    ...
    Double bitSwapProbability = (Double)getParameter("bitSwapProbability");
    if (parents.length < 2)
    {
      Configuration.logger_.severe("UXCrossover.execute: operator needs two " +
          "parents");
      Class cls = java.lang.String.class;
      String name = cls.getName(); 
      throw new JMException("Exception in " + name + ".execute()") ;      
    ...
    {
      Configuration.logger_.severe("UXCrossover.execute: probability not " +
      "specified");
      Class cls = java.lang.String.class;
      String name = cls.getName(); 
      throw new JMException("Exception in " + name + ".execute()") ;  
    ...
    Solution [] offSpring = doCrossover(probability.doubleValue(),
    							 bitSwapProbability.doubleValue(),
                                                       parents[0],
                                                       parents[1]);
