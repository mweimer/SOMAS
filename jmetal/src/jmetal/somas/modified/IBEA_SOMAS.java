 ...
 */
public class IBEA_SOMAS extends Algorithm{
  ...
  */
  public IBEA_SOMAS(SOMASProblem problem) {
  ...
  */
  public SolutionSet execute() throws JMException, ClassNotFoundException{
    ...
    ArrayList<Solution> solutionsToBeEvaluated = new ArrayList<Solution>();
    boolean useJPPF;
    ...
    useJPPF = ((Boolean) getInputParameter("useJPPF")).booleanValue();
    ...
    int genCount = 0;
    System.out.println("Creating generation " + genCount + ".");
    Solution newSolution;
    for (int i = 0; i < populationSize; i++) {
      newSolution = new Solution(problem_);
      if(!useJPPF)
      {
    	  System.out.println("Evaluating generation " + genCount + ", individual " + i + ".");
	      problem_.evaluate(newSolution);
	      problem_.evaluateConstraints(newSolution);
      }
      else
    	  solutionsToBeEvaluated.add(newSolution);
      evaluations++;
      solutionSet.add(newSolution);
    }
    if(useJPPF)
    {
    	problem_.parallelEvaluate(solutionsToBeEvaluated);
      	solutionsToBeEvaluated.clear();
    }
    genCount++;
    ...
    while (evaluations < maxEvaluations){
      ...
      while (offSpringSolutionSet.size() < populationSize){
        ...
        } while (j < IBEA_SOMAS.TOURNAMENTS_ROUNDS); // do-while
        int k = 0;
        do{
          k++;
          parents[1] = (Solution)selectionOperator.execute(archive);
        } while (k < IBEA_SOMAS.TOURNAMENTS_ROUNDS); // do-while
        ...
        if(!useJPPF)
        {
        	...
        	solutionsToBeEvaluated.add(offSpring[0]);
