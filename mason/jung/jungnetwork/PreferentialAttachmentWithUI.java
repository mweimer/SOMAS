...
public class PreferentialAttachmentWithUI extends GUIState {
		...
		Steppable histUpdater = new Steppable() {
			...
			public void step(SimState state) {
				if (degreesFrame.isVisible()) {
					...
					degrees = new double[((PreferentialAttachment) state).graph
							.numVertices()];

					Iterator i = ((PreferentialAttachment) state).graph
	...
	public void init(Controller c) {
		...
		degreesChart.addSeries(this.degrees,
				((PreferentialAttachment) state).maxDegree, "Degree histogram",
				null);
		degreesChart.update();
