...
public class DDoSClassification extends Scenario {
	
	public static final int numObjectives = 5; // TODO : change back to 6, set to 5 right now for backwards compatibility
	public static final String[] metricNames = {"activity metric", "targets taken down", "DDoS agents deleted", 
		"self organization", "feasibility",  "dead nodes"};
	
	public DDoSClassification() {super();}

	public static void main(String[] args) { new DDoSClassification().visualize("", false); }
    ...
    private void setupSimulation() {
		...
		flatNetwork.addDDoSAgents(4, 10);
	...
	public void run() {
		...
		objectiveValueContainers.add((new Thing()).setProperty("object", new Double(0))); // targets taken down
		objectiveValueContainers.add((new Thing()).setProperty("object", new Double(0))); // DDoS agents deleted
		...
		// Calculate the number of targets destroyed
		Double targetsDestroyed = 0.0;
		for(Location target : ((FlatNetwork) network).getTargetList())
			if(target.hasBoxedProperty("isVitalNode") && (Boolean) target.getBoxedProperty("isVitalNode"))
				targetsDestroyed++;
		objectiveValueContainers.get(1).setProperty("object", targetsDestroyed);
